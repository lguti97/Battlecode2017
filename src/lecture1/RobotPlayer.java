...
public strictfp class RobotPlayer {
    ...
    public static void trytoBuildSoldier(RobotType type, int moneyNeeded) throws GameActionException{
        //try to build soldiers
        //can I build a soldier
        if (rc.getTeamBullets() > moneyNeeded) {
            //get executed if we have enough bullets to build the unit
            for (int i = 0; i < 4; i++) {
                if (rc.canBuildRobot(type, dirList[i])) {
                    rc.buildRobot(type, dirList[i]);
                    break;
